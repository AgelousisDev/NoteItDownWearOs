object Versions {
    const val GRADLE_PLUGIN = "8.11.0"
    const val KOTLIN_VERSION = "2.2.0-Beta2"
    const val COMPOSE_VERSION = "1.9.0-alpha04"
    const val COMPOSE_ACTIVITY_VERSION = "1.12.0-alpha01"
    const val CORE_KTX_VERSION = "1.16.0"
    const val WEAR_COMPOSE_VERSION = "1.5.0-alpha13"
    const val WEAR_INPUT_VERSION = "1.2.0-alpha02"
    const val LIVE_DATA_VIEW_MODEL_VERSION = "2.9.0"
    const val DATA_STORE_PREFERENCE_VERSION = "1.1.1"
    const val CONSTRAINT_LAYOUT_COMPOSE_VERSION = "1.1.1"
    const val LIFECYCLE_RUNTIME_KTX_VERSION = "2.9.0"
    const val GLANCE_WEAR_TILE_VERSION = "1.0.0-alpha05"
    const val RETROFIT_CONVERTER_GSON_VERSION = "3.0.0"
    const val HTTP_LOGGING_INTERCEPTOR_VERSION = "5.0.0-alpha.14"
    const val COMPOSE_MATERIAL_ICONS_VERSION = "1.7.8"

    const val KOTLIN_COMPILER_EXTENSION_VERSION = "1.5.15"
    const val GLANCE_VERSION = "1.0.0-alpha05"
    const val GLANCE_APP_WIDGET_VERSION = "1.0.0-alpha05"
    const val GITHUB_BANES_VERSION = "0.52.0"
    const val WEAR_TOOLING_PREVIEW_DEVICES_VERSION = "1.0.0"
    const val COIL_COMPOSE_VERSION = "2.7.0"
    const val WEAR_COMPOSE_MATERIAL_YOU_VERSION = "1.5.0-beta02"
}
object ConfigData {
    const val COMPILE_SDK_VERSION = 36
    const val MIN_SDK_VERSION = 28
    const val TARGET_SDK_VERSION = 36
    const val VERSION_CODE = 1
    const val VERSION_NAME = "1.0"
    const val PACKAGE_NAME = "com.agelousis.noteitdown"
}